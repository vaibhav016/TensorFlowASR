...
batch_size = None or config.learning_config.running_config.batch_size
test_data_loader = test_dataset.create(batch_size)

m1 = [contextnet.layers[0]]

for base_model in m1:
    '''
    ...
    # inputs["inputs"], inputs["inputs_length"], inputs["signal"]
    inputs = np.ones(shape=(1,87,80,1))
    inputs_length = np.ones(shape=(1,))
    predictions = np.ones(shape=(1,))
    predictions_length = np.ones(shape=(1,))
    signal = np.ones(shape = tf.TensorShape((1, 200, 1)))

    feat_contextnet = base_model.call_feature_output([inputs, inputs_length, signal])
    print(3)
    # feat = newmodel.predict(input_)
    base_weights = base_model.get_weights()
    out_ = feat_contextnet.dot(base_weights[-2]) + base_weights[-1]
    '''
